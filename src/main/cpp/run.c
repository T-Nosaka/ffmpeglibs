 ...
 */
static int transcode(Scheduler *sch, void* pext, int (*callback)( void* pext, int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts) )
{
    int ret = 0;
    ...
    while (!sch_wait(sch, stats_period, &transcode_ts)) {
        ...
        if( callback( pext, 0, timer_start, cur_time, transcode_ts) != 0 )
            break;
...
void output_report(Report2* rep, int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts) {
    ...
    rep->duration = (*input_files)->ctx->duration + ((*input_files)->ctx->duration <= INT64_MAX - 5000 ? 5000 : 0);
    rep->nb_input_files = nb_input_files;
    rep->nb_output_files = nb_output_files;
    rep->nb_filtergraphs = nb_filtergraphs;
    rep->nb_decoders = nb_decoders;

    if( rep->osts == NULL )
        rep->osts = malloc(sizeof(Report2_ost)*(rep->nb_output_files+1));

    AVBPrint buf, buf_script;
    int64_t total_size = of_filesize(output_files[0]);
    rep->total_size = total_size;
...
//    av_bprint_init(&buf_script, 0, AV_BPRINT_SIZE_AUTOMATIC);
    int ostcnt=0;
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost),ostcnt++)
    {
        const float q = ost->enc ? atomic_load(&ost->quality) / (float)FF_QP2LAMBDA : -1;

        rep->osts[ostcnt].q = q;
        rep->osts[ostcnt].mediatype = ost->type;
        rep->osts[ostcnt].vid = vid;
        rep->osts[ostcnt].outfilter = ost->filter;
...
//                       ost->file->index, ost->index, q);
        ...
        if (!vid && ost->type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            rep->osts[ostcnt].frame_number = frame_number;
            rep->osts[ostcnt].fps = fps;
...
//                av_bprintf(&buf, "L");
            ...
            if (ost->filter)
            {
                ...
                rep->osts[ostcnt].nb_frames_dup = nb_frames_dup;
                rep->osts[ostcnt].nb_frames_drop = nb_frames_drop;
            }
...
void filedump( const char *filename, cJSON* root )
{
    ...
    for (unsigned int i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        if(cd)
            ...
            cJSON_AddNumberToObject(stream, "codec", par->codec_id);
...
//    current_time = ti = get_benchmark_time_stamps();
    ret = transcode(sch, pext, callback );
